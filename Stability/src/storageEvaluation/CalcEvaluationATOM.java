...
public class CalcEvaluationATOM {
	
	private ArrayList<Evaluation> evaluations;
	private ArrayList<LoadedValue> load;
	
	public CalcEvaluationATOM(ArrayList<LoadedValue> load){
		this.load=load;
	...
	private void shortenEvaluations(Calendar fromDate, Calendar toDate) {
		...
		if (evaluations.size()>0){
			if (fromDate==null){
				...
				fromDate.set(Calendar.YEAR, evaluations.get(0).getDate().get(Calendar.YEAR));
				fromDate.set(Calendar.MONTH, evaluations.get(0).getDate().get(Calendar.MONTH));
				fromDate.set(Calendar.DAY_OF_MONTH, evaluations.get(0).getDate().get(Calendar.DAY_OF_MONTH));
			}
			if (toDate==null){
